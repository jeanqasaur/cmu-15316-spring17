...
type cmd =
  ...
  | Set_delegate of ident * right * principal
  | Delete_delegate of ident * right * principal  
  | Return of expr
  | Exit
type prog =
...
let string_of_cmd c =
  match c with
    ...
    | Set_delegate (x,r,p) -> "set delegation "^x^" "^ (string_of_right r)^" "^(string_of_principal p)
    | Delete_delegate (x,r,p) -> "delete delegation "^x^" "^ (string_of_right r)^" "^(string_of_principal p)
